...
if(isset($_POST['patsub1'])){
	...
	// SSE-SECURITY-FIX: secure patient registration with prepared statement and password hashing.
	$fname=sse_clean_input($_POST['fname']);
  $lname=sse_clean_input($_POST['lname']);
  $gender=sse_clean_input($_POST['gender']);
  $email=sse_clean_input($_POST['email']);
  $contact=sse_clean_input($_POST['contact']);
	$password=sse_clean_input($_POST['password']);
  $cpassword=sse_clean_input($_POST['cpassword']);
  if($password==$cpassword){
    $hashedPassword = sse_hash_password_if_needed($password);
    ...
    $query1 = "SELECT pid FROM patreg WHERE email = ? ORDER BY pid DESC LIMIT 1";
    $stmtPid = mysqli_prepare($con, $query1);
    mysqli_stmt_bind_param($stmtPid, "s", $email);
    mysqli_stmt_execute($stmtPid);
    $result1 = mysqli_stmt_get_result($stmtPid);
