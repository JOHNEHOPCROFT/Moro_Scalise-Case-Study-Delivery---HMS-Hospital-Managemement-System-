...
if(isset($_POST['patsub'])){
	// SSE-SECURITY-FIX: CSRF validation for patient login.
	if (!sse_verify_csrf_token($_POST['csrf_token'] ?? '')) {
		die('Invalid CSRF token.');
	}
	// SSE-SECURITY-FIX: sanitize input and replace vulnerable login query with prepared statement.
	$email=sse_clean_input($_POST['email']);
...
if(isset($_POST['update_data']))
{
	...
	// SSE-SECURITY-FIX: use prepared statement for payment status update.
...
if(isset($_POST['doc_sub']))
{
	...
	// SSE-SECURITY-FIX: secure doctor creation with hashing and prepared statement.
  ...
  $hashedDoctorPassword = sse_hash_password_if_needed($dpassword);
	$stmt = mysqli_prepare($con, "INSERT INTO doctb(username,password,email,docFees) VALUES(?,?,?,?)");
	mysqli_stmt_bind_param($stmt, "sssi", $doctor, $hashedDoctorPassword, $demail, $docFees);
	$result=mysqli_stmt_execute($stmt);
	if($result)
