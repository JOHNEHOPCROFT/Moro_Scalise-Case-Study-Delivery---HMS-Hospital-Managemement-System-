...
sse_require_authenticated_patient('index1.php');
include('func.php');  
include('newfunc.php');
...
if(isset($_POST['app-submit']))
{
  ...
  $gender = $_SESSION['gender'];
  $contact = $_SESSION['contact'];
  $doctor=sse_clean_input($_POST['doctor']);
  $email=$_SESSION['email'];
  # $fees=$_POST['fees'];
  ...
  if(date("Y-m-d",$appdate1)>=$cur_date){
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      $checkStmt = mysqli_prepare($con, "SELECT apptime FROM appointmenttb WHERE doctor=? AND appdate=? AND apptime=?");
      mysqli_stmt_bind_param($checkStmt, "sss", $doctor, $appdate, $apptime);
      mysqli_stmt_execute($checkStmt);
      $check_query = mysqli_stmt_get_result($checkStmt);

        if(mysqli_num_rows($check_query)==0){
          $insertStmt = mysqli_prepare($con, "INSERT INTO appointmenttb(pid,fname,lname,gender,email,contact,doctor,docFees,appdate,apptime,userStatus,doctorStatus) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
          $userStatus = 1; $doctorStatus = 1;
          mysqli_stmt_bind_param($insertStmt, "issssssissii", $pid, $fname, $lname, $gender, $email, $contact, $doctor, $docFees, $appdate, $apptime, $userStatus, $doctorStatus);
          $query=mysqli_stmt_execute($insertStmt);
...
if(isset($_POST['cancel_appointment']))
  {
    // SSE-SECURITY-FIX: protect patient-side cancellation with POST + CSRF.
    if (!sse_verify_csrf_token($_POST['csrf_token'] ?? '')) {
      die('Invalid CSRF token.');
    }
    $appointmentId = (int) sse_clean_input($_POST['appointment_id'] ?? '0');
...
function generate_bill(){
  ...
  $pid = $_SESSION['pid'];
  $output='';
  $appointmentId = (int) sse_clean_input($_GET['ID'] ?? '0');
  ...
  mysqli_stmt_bind_param($stmt, "ii", $pid, $appointmentId);
  mysqli_stmt_execute($stmt);
  $query=mysqli_stmt_get_result($stmt);
  while($row = mysqli_fetch_array($query)){
    $output .= '
    <label> Patient ID : </label>'.sse_e($row["pid"]).'<br/><br/>
    <label> Appointment ID : </label>'.sse_e($row["ID"]).'<br/><br/>
    <label> Patient Name : </label>'.sse_e($row["fname"]).' '.sse_e($row["lname"]).'<br/><br/>
    <label> Doctor Name : </label>'.sse_e($row["doctor"]).'<br/><br/>
    <label> Appointment Date : </label>'.sse_e($row["appdate"]).'<br/><br/>
    <label> Appointment Time : </label>'.sse_e($row["apptime"]).'<br/><br/>
    <label> Disease : </label>'.sse_e($row["disease"]).'<br/><br/>
    <label> Allergies : </label>'.sse_e($row["allergy"]).'<br/><br/>
    <label> Prescription : </label>'.sse_e($row["prescription"]).'<br/><br/>
    <label> Fees Paid : </label>'.sse_e($row["docFees"]).'<br/>
...
if(isset($_POST["generate_bill"])){
  // SSE-SECURITY-FIX: protect bill generation with POST + CSRF.
  if (!sse_verify_csrf_token($_POST['csrf_token'] ?? '')) {
    die('Invalid CSRF token.');
  }
...
.btn-primary{
  ...
  <body style="padding-top:50px;">
   ...
   <div class="container-fluid" style="margin-top:50px;">
    <h3 style = "margin-left: 40%;  padding-bottom: 20px; font-family: 'IBM Plex Sans', sans-serif;"> Welcome &nbsp<?php echo sse_e($username) ?> 
   </h3>
    <div class="row">
...
<div class="tab-pane fade" id="app-hist" role="tabpanel" aria-labelledby="list-pat-list">
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  ...
                  ?>
                      <tr>
                        <td><?php echo sse_e($row['doctor']);?></td>
                        <td><?php echo sse_e($row['docFees']);?></td>
                        <td><?php echo sse_e($row['appdate']);?></td>
                        <td><?php echo sse_e($row['apptime']);?></td>
                    ...
                    if(($row['userStatus']==1) && ($row['doctorStatus']==0))  
                    {
                      echo "Cancelled by Doctor";
                        ...
                        { ?>
	                        ...
	                        <form method="post" action="admin-panel.php" style="display:inline;" onsubmit="return confirm('Are you sure you want to cancel this appointment ?');">
	                          <input type="hidden" name="csrf_token" value="<?php echo sse_e(sse_generate_csrf_token()) ?>">
	                          <input type="hidden" name="appointment_id" value="<?php echo sse_e($row['ID']) ?>">
	                          <button type="submit" name="cancel_appointment" class="btn btn-danger" title="Cancel Appointment">Cancel</button>
	                        </form>
	                        <?php } else {
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  ...
                  ?>
                      <tr>
                        <td><?php echo sse_e($row['doctor']);?></td>
                        <td><?php echo sse_e($row['ID']);?></td>
                        <td><?php echo sse_e($row['appdate']);?></td>
                        <td><?php echo sse_e($row['apptime']);?></td>
                        <td><?php echo sse_e($row['disease']);?></td>
                        <td><?php echo sse_e($row['allergy']);?></td>
                        <td><?php echo sse_e($row['prescription']);?></td>
                        <td>
                          ...
                          <!-- <a href="admin-panel.php?ID=" 
                              ...
                              <input type="hidden" name="csrf_token" value="<?php echo sse_e(sse_generate_csrf_token()) ?>">
                              <input type="hidden" name="ID" value="<?php echo sse_e($row['ID'])?>"/>
      ...
      <div class="tab-pane fade" id="list-settings" role="tabpanel" aria-labelledby="list-settings-list">
        <form class="form-group" method="post" action="func.php" autocomplete="off">
